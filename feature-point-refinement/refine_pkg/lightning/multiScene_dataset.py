...
from refine_pkg.utils.dataloader import get_local_split
from refine_pkg.utils.misc import tqdm_joblib
from refine_pkg.datasets.megadepth import MegaDepthDataset
from refine_pkg.datasets.sampler import RandomConcatSampler
...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        assert stage in ['fit', 'test', 'validate'], f"stage must be either fit or test, now {stage}"
    ...
    def val_dataloader(self):
        ...
        logger.info(f'Val Sampler and DataLoader re-init.')
        if not isinstance(self.val_dataset, abc.Sequence):
            # 移除 DistributedSampler，直接使用 DataLoader
            return DataLoader(self.val_dataset, **self.val_loader_params)
        else:
            dataloaders = []
            for dataset in self.val_dataset:
                # 对于序列中的每个数据集，也移除 DistributedSampler 的使用
                dataloaders.append(DataLoader(dataset, **self.val_loader_params))
            return dataloaders

    def test_dataloader(self, *args, **kwargs):
        logger.info(f'Test Sampler and DataLoader re-init.')
        # 同样移除 DistributedSampler 的使用
...
if __name__ == "__main__":
    import argparse
    from refine_pkg.default import get_cfg_defaults
    from refine_pkg.configs.data.megadepth_trainval import cfg
    def parse_args():
